...
- [ ] 한국에서 자동차 길찾기를 지원하지 않는 Google 지도 경로 링크를 카카오맵 자동차 길찾기로 교체한다.
  - 참고: [카카오 지도 길찾기 바로가기](https://apis.map.kakao.com/web/guide/)
- [ ] 일차별 출발지와 도착지 응답에 장소명뿐 아니라 위도·경도를 포함한다.
- [ ] 출발지, 일정 장소, 도착지를 기존 일정 순서대로 카카오맵 경로에 전달한다.
- [ ] 카카오맵의 경유지 최대 5개 제한을 넘는 일정은 경로를 분할하거나 사용자에게 제한을 안내한다.
- [ ] 출발지나 도착지 좌표가 없을 때 기존 장소 보기 또는 내부 지도 화면으로 안전하게 대체한다.
- [ ] PC와 모바일 웹에서 자동차 이동수단, 경유지 순서, 앱 미설치 시 동작을 직접 확인한다.
- [ ] 외부 지도 링크 생성 규칙과 경유지 제한 처리 테스트를 추가한다.
